...
**The preferred language also selects the audio track inside the file.** Multi-audio releases carry several tracks and both players default to the first one, which is rarely the wanted one. The language list is converted to the codes containers actually tag tracks with — three-letter ISO 639-2 first, since Matroska and MP4 almost always use that form, with two-letter as fallback — and passed as `--alang` to mpv and `--audio-language` to VLC. The same happens for subtitles via `--slang` / `--sub-language`. This is independent of which release was chosen: picking the source stays the user's decision.
...
- **mpv does it properly.** `--vo=gpu-next` (libplacebo) plus `--gpu-api=d3d11` and `--hwdec=auto-safe`, then a passthrough hint and a tone-mapping curve — which are **independent controls**, not an either/or. An HDR display still needs a curve for whatever peak brightness it cannot physically reach, so `target-colorspace-hint=yes` and `tone-mapping=bt.2446a` together is a valid and useful combination; an earlier version treated choosing a curve as turning passthrough off, which silently degraded HDR output. `clip` leaves the mapping entirely to the display. Dolby Vision needs no extra flag — libplacebo applies the dynamic metadata itself under `gpu-next`, and mpv exposes no DV-specific option.
...
**Subtitles can be injected mid-playback on mpv.** At launch both players get `--sub-file`. While something is already playing, mpv will also take a track over its IPC channel (`sub-add … select`), so a different language can be dropped in without restarting the stream. VLC's HTTP interface has no equivalent, so there the track must be chosen before pressing play — the UI only offers the button for mpv.
...
- **Audio track selection**, against a purpose-built 3-track file (`eng`, `tur`, `ger`, English deliberately first so "first track" and "preferred track" differ): no preference selects `aid 1` / `eng`; Turkish preferred selects `aid 2` / `tur`; German selects `aid 3` / `ger`; a language the file lacks falls back to a real track rather than silence. Every language the release parser can produce maps to at least one track code. VLC registers the preference too (`selected audio language[0] tr`)
